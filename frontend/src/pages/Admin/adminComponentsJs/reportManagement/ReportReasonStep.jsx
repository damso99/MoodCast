...
export function ReportReasonStep({ actionMeta, selectedReason, reasonDetail, onBack, onClose, onChangeReason, onChangeDetail, onNext }) {
  return (
    <>
      <DrawerHeader title={`${actionMeta?.label || '제재'} 사유 설정`} onBack={onBack} onClose={onClose} />

      <div className={styles.drawerBody}>
        <p className={styles.guideText}>관리자가 어떤 이유로 처리하는지 확인할 수 있도록 사유를 남깁니다.</p>

        {/* 경고/영구 정지 사유 입력 ---------------------------------- */}
        <section className={styles.formSection}>
          <label htmlFor="reason-select">제재 사유 *</label>
          <select id="reason-select" value={selectedReason} onChange={(event) => onChangeReason(event.target.value)}>
