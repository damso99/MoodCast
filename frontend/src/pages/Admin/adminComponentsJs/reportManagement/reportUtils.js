...
export function countReportsByStatus(reports, statusTabs) {
  return statusTabs.reduce((counts, tabLabel) => {
    counts[tabLabel] = tabLabel === '전체' ? reports.length : reports.filter((report) => report.status === tabLabel).length;
    return counts;
  }, {});
}
...
export function countReportsByType(reports, typeTabs) {
  return typeTabs.reduce((counts, tabLabel) => {
    counts[tabLabel] = tabLabel === '전체' ? reports.length : reports.filter((report) => report.type === tabLabel).length;
    return counts;
  }, {});
}
