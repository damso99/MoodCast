...
import { reportStatusTabs, reportTypeTabs, sanctionOptions, todayText } from './reportConstants';
import { countReportsByStatus, countReportsByType, getReleaseDate } from './reportUtils';
...
export function ReportManagementPage() {
  ...
  const filteredReports = useMemo(() => {
    return reports.filter((report) => {
      const statusMatched = selectedStatusTab === '전체' || report.status === selectedStatusTab; // 상태 탭 조건입니다.
      const typeMatched = selectedTypeTab === '전체' || report.type === selectedTypeTab; // 유형 탭 조건입니다.
      return statusMatched && typeMatched; // 두 조건을 모두 만족하는 신고만 목록에 보여줍니다.
    });
  }, [reports, selectedStatusTab, selectedTypeTab]);

  const selectedActionMeta = sanctionOptions.find((option) => option.id === selectedAction); // 선택한 제재 옵션의 라벨/설명을 찾습니다.
  const releaseDate = getReleaseDate(selectedPeriod === 'custom' ? Number(customPeriod) || 'custom' : selectedPeriod); // 일시 정지 예상 해제 일시입니다.
  const statusCounts = useMemo(() => countReportsByStatus(reports, reportStatusTabs), [reports]); // 상태 탭에 표시할 신고 개수입니다.
  const typeCounts = useMemo(() => countReportsByType(reports, reportTypeTabs), [reports]); // 유형 탭에 표시할 신고 개수입니다.

  const completionToast = completionMessage ? (
    <ReportCompletionToast message={completionMessage} onClose={() => setCompletionMessage('')} />
  ) : null; // 제재/반려 완료 후 상단에 완료 메시지를 보여주는 컴포넌트입니다.

  const statusTabs = (
    <ReportStatusTabs tabs={reportStatusTabs} selectedTab={selectedStatusTab} counts={statusCounts} onSelect={setSelectedStatusTab} />
  ); // 전체/처리 대기/검토 중/처리 완료/반려 상태 필터를 보여주는 컴포넌트입니다.

  const typeTabs = (
    <ReportTypeTabs tabs={reportTypeTabs} selectedTab={selectedTypeTab} counts={typeCounts} onSelect={setSelectedTypeTab} />
  ...
  const reportDrawer = selectedReport ? (
    <ReportDrawer
      ...
      onBackFromConfirm={() => (selectedAction === 'temporary' ? setPanelStep('temporary') : setPanelStep(selectedAction === 'reject' ? 'option' : 'reason'))}
      onConfirm={confirmSanction}
    />
  ) : null; // 신고 상세 확인부터 제재 확정까지 오른쪽에서 단계별로 보여주는 사이드 패널 컴포넌트입니다.

  const resultModal = selectedResultReport ? (
    <ReportResultModal report={selectedResultReport} onClose={() => setSelectedResultReport(null)} />
  ...
  function openDetailPanel(report) {
    ...
    const nextReport = report.status === '처리 대기' ? { ...report, status: '검토 중' } : report; // 처음 클릭한 신고는 검토 중으로 바꿉니다.

    if (report.status === '처리 대기') {
      setReports((prevReports) => prevReports.map((item) => (item.id === report.id ? nextReport : item))); // 목록 상태도 검토 중으로 갱신합니다.
    }
  ...
  function moveNextFromOption() {
    ...
    if (selectedAction === 'temporary') setPanelStep('temporary'); // 일시 정지는 기간 설정 화면으로 이동합니다.
    else if (selectedAction === 'reject') setPanelStep('confirm'); // 반려는 별도 설정 없이 최종 확인으로 이동합니다.
  ...
  function confirmSanction() {
    ...
    const periodLabel = selectedPeriod === 'custom' ? `${customPeriod || 0}일` : `${selectedPeriod}일`; // 결과 조회에 보여줄 정지 기간입니다.
    const sanctionResult = {
      actionLabel: selectedActionMeta?.label,
      reason: selectedAction === 'reject' ? '반려' : selectedReason,
      detail: selectedAction === 'reject' ? '신고가 부적절하다고 판단되어 반려합니다.' : reasonDetail || selectedReport.detail,
  ...
  return (
    <AdminLayout title="신고 및 제재 관리" description="신고된 콘텐츠와 사용자를 검토하고 적절한 제재를 관리합니다.">
      {completionToast}

      <section className={styles.topTabs} aria-label="신고 관리 상위 탭">
        {['신고 목록', '제재 이력', '통계'].map((label) => (
          <button key={label} className={label === '신고 목록' ? styles.activeTopTab : ''} type="button">
            {label}
          </button>
        ))}
      </section>
